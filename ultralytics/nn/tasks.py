...
class DetectionModel(BaseModel):
    ...
    def _predict_augment(self, x):
        ...
        y = []  # outputsfrom torch.xpu import devicefrom torch.xpu import devicefrom torch.xpu import devicefrom torch.xpu import devicefrom torch.xpu import devicefrom torch.xpu import devicefrom torch.xpu import device
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [*c1, c2]
        elif m is CGAupsample:
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [max(ch[f[0]], ch[f[1]]), min(ch[f[0]], ch[f[1]]), max(ch[f[0]], ch[f[1]])]
        elif m is CBFuse:
            c2 = ch[f[-1]]
        elif m is CGAAttention:
            c1 = c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args=[c1]
        else:
            c2 = ch[f]
